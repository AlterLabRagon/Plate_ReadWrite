...
check_column_name <- function(file,filename,column_names,variable_column) {
  data_key <- read.csv(file,header=TRUE)
  if(!all(column_names %in% tolower(colnames(data_key))))
  {warning(paste0(filename," must have columns named ",paste(column_names,collapse = ', '), ". Current columns include: ", paste(colnames(data_key),collapse = ', ')))
    if(ifelse(any(grepl(variable_column,colnames(data_key))),TRUE,FALSE) == FALSE)
...
################################################################################
check_annotated_data_demo <- function(file) {
...
#  check("Checking annotated meta data file path", function() check_file_path(file))
  ...
  check("Checking valid annotated meta data column labels", function() check_column_name(file,'Annotated Data','unique_id','unique_id'))
...
################################################################################
check_exp_data_demo <- function(file,measurements,num_384,num_96) {
  check <- function(description, f) {
    message(paste0("* ", description, " ... ", collapse = ""), appendLF = FALSE)
    result <- tryCatch(
      expr = f(), 
...
################################################################################
check_data_key_demo <- function(file) {
  check <- function(description, f) {
    message(paste0("* ", description, " ... ", collapse = ""), appendLF = FALSE)
    result <- tryCatch(
      expr = f(), 
...
#  check("Checking that data key file is a csv", function() check_file_path(file))
  ...
  columns <- check("Checking data key has valid column labels", function() check_column_name(file,'Data key',c("plate","platemap","set","secondary"),'platemap'))
...
{if(nrow(pre_combined)  > nrow(combined))
{warning(paste('IDs dropped or not found in experimental data!', '\n','Number of dropped IDs: ',length(pre_combined$unique_id_edit[ !pre_combined$unique_id_edit%in%combined$unique_id_edit]),'\n','Check the well and plate numbers of dropped IDs in the dropped ID csv file'))
  dropped_ids <- as.data.frame(pre_combined[ !pre_combined$unique_id_edit %in% combined$unique_id_edit, 1:3],colnames = c('Plate','Well.ID','unique_id_edit'))
